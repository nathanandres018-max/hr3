...
/**
 * flagged_claims.php
 * Displays flagged claims requiring manual review
 * Aligns with pending_claims.php and processed_claims.php
 */
...
// Build where clauses
$where_clauses = ["c.status IN ('flagged', 'needs_manual_review')"];
...
if (!empty($search_query)) {
    ...
    $where_clauses[] = "(c.vendor LIKE '%$search%' OR e.fullname LIKE '%$search%' OR c.id LIKE '%$search%')";
}

$where = implode(' AND ', $where_clauses);

// Validate sort
$allowed_sorts = ['c.created_at', 'c.amount', 'c.category', 'c.vendor'];
...
$cat_sql = "SELECT DISTINCT category FROM claims WHERE category IS NOT NULL AND category != '' AND status IN ('flagged', 'needs_manual_review') ORDER BY category ASC";
...
function render_sidebar(string $active = 'flagged_claims') {
    $links = [
...
<head>
    <meta charset="UTF-8">
    <title>Flagged Claims - ViaHale TNVS HR3</title>
...
<div class="layout">
    ...
    <div class="sidenav col-auto p-0">
        <?php render_sidebar('flagged_claims'); ?>
    </div>
    ...
    <div class="main-content">
        ...
        <div class="topbar">
            <div>
                <h1>
                    <ion-icon name="alert-circle-outline"></ion-icon> Flagged Claims
                </h1>
                <p class="topbar-subtitle">Claims requiring manual review and action</p>
            </div>
            <div class="profile">
        ...
        <div class="page-content">
            ...
            <div class="stats-section">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value"><?= count($flagged_claims) ?></div>
                </div>
                <div class="stat-card">
                    <div class="stat-label">Total Value</div>
                    <div class="stat-value">₱<?= number_format(array_sum(array_column($flagged_claims, 'amount')) ?: 0, 2) ?></div>
                </div>
            </div>

            <!-- Filter Card -->
            <div class="card mb-3">
                <div class="card-header" style="background: linear-gradient(90deg, #9A66ff 0%, #4311a5 100%);">
                ...
                <div class="card-body">
                    <form method="GET" class="filter-section">
                        ...
                        <div class="filter-buttons">
                            ...
                            <a href="flagged_claims.php" class="btn btn-secondary btn-sm">
            ...
            <div class="card">
                <div class="card-header">
                    <ion-icon name="list-outline"></ion-icon> Flagged Claims List
                </div>
                <div class="card-body">
                    <?php if (empty($flagged_claims)): ?>
                        <div class="empty-state">
                            <ion-icon name="checkmark-circle-outline"></ion-icon>
                            <h5>No Flagged Claims</h5>
                            <p>Great! No claims are currently flagged for review.</p>
                        </div>
                    <?php else: ?>
                        <div style="overflow-x: auto;">
                            <table class="claims-table">
                                ...
                                <tbody>
                                    <?php foreach ($flagged_claims as $claim): ?>
                                        <tr>
                                            <td><span class="claim-id">#<?= htmlspecialchars($claim['id']) ?></span></td>
                                            <td>
                                                <div><?= htmlspecialchars($claim['employee_name'] ?? 'Unknown') ?></div>
                                            ...
                                            <td>
                                                <span class="status-badge status-<?= str_replace('_', '-', strtolower($claim['status'])) ?>">
                                                    <?= htmlspecialchars(ucfirst(str_replace('_', ' ', $claim['status']))) ?>
                                                </span>
                                            </td>
                                            <td>
                                                <div class="action-buttons">
                                                    <button type="button" class="btn btn-sm btn-approve btn-action" data-action="approve" data-claim="<?= htmlspecialchars($claim['id']) ?>">
                                                    ...
                                                    <?php if (!empty($claim['receipt_path'])): ?>
                                                        <a class="btn btn-sm btn-outline-dark" href="<?= htmlspecialchars($claim['receipt_path']) ?>" target="_blank" rel="noopener">
                                                            <ion-icon name="download-outline"></ion-icon> Receipt
                                                        </a>
                                                    <?php endif; ?>
                                                </div>
                                            </td>
