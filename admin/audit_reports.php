<?php
/**
 * admin/audit_reports.php
 * Admin view — Audit & Reports with statistics, filters, CSV export
 */
...
// Build WHERE
$where_clauses = ["DATE(c.created_at) >= '" . mysqli_real_escape_string($conn, $from_date) . "'"];
$where_clauses[] = "DATE(c.created_at) <= '" . mysqli_real_escape_string($conn, $to_date) . "'";

if (!empty($claim_type)) {
    $where_clauses[] = "c.category = '" . mysqli_real_escape_string($conn, $claim_type) . "'";
}
if (!empty($status_filter)) {
    $where_clauses[] = "c.status = '" . mysqli_real_escape_string($conn, $status_filter) . "'";
}

$where = implode(' AND ', $where_clauses);

// Fetch claims for report
...
foreach ($claims as $c) {
    $total_amount += (float)($c['amount'] ?? 0);
    $st = strtolower($c['status'] ?? '');
    if ($st === 'approved' || $st === 'processed') $approved_count++;
    if ($st === 'pending') $pending_count++;
    if ($st === 'flagged') $flagged_count++;
    if (!empty($c['reviewed_by'])) $reviewed_count++;
}

// Get categories & statuses for filters
$cat_result = mysqli_query($conn, "SELECT DISTINCT category FROM claims WHERE category IS NOT NULL AND category != '' ORDER BY category ASC");
$categories = [];
while ($row = mysqli_fetch_assoc($cat_result)) {
    if (!empty($row['category'])) $categories[] = $row['category'];
}

$stat_result = mysqli_query($conn, "SELECT DISTINCT status FROM claims WHERE status IS NOT NULL AND status != '' ORDER BY status ASC");
$statuses = [];
while ($row = mysqli_fetch_assoc($stat_result)) {
    if (!empty($row['status'])) $statuses[] = $row['status'];
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <title>Audit & Reports — HR3 Admin</title>
  ...
  <style>
    * { transition: all 0.3s ease; }
    ...
    .stats-section { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem; margin-bottom: 1.5rem; }
    ...
    .stat-card label { font-size: 0.8rem; color: #6c757d; font-weight: 600; margin-bottom: 0.5rem; display: block; }
    .stat-card .value { font-size: 1.6rem; font-weight: 800; color: #22223b; }
    .stat-card.approved { border-left-color: #10b981; }
    .stat-card.approved .value { color: #10b981; }
    .stat-card.pending { border-left-color: #f59e0b; }
    .stat-card.pending .value { color: #f59e0b; }
    .stat-card.flagged { border-left-color: #ef4444; }
    .stat-card.flagged .value { color: #ef4444; }
    .stat-card.reviewed { border-left-color: #3b82f6; }
    .stat-card.reviewed .value { color: #3b82f6; }
    ...
    .amount-badge { display: inline-block; padding: 0.3rem 0.7rem; border-radius: 20px; font-size: 0.82rem; font-weight: 700; background: #d1fae5; color: #065f46; }
    .status-badge { display: inline-block; padding: 0.3rem 0.7rem; border-radius: 20px; font-size: 0.82rem; font-weight: 600; }
    .status-approved, .status-processed { background: #d1fae5; color: #065f46; }
...
<div class="wrapper">
    ...
    <div>
      ...
      <div class="mb-4">
        ...
        <nav class="nav flex-column">
          ...
          <a class="nav-link active" href="audit_reports.php"><ion-icon name="document-text-outline"></ion-icon>Audit & Reports</a>
        </nav>
      </div>
    </div>
  ...
  <div class="content-wrapper">
    <div class="topbar">
      <h1 class="dashboard-title">Audit & Reports</h1>
      <div class="profile">
    ...
    <div class="main-content">
      ...
      <div class="card">
        <div class="card-header">
          <ion-icon name="funnel-outline"></ion-icon> Report Filters
        </div>
        <div class="card-body">
          <form method="GET" class="filters-section">
            <div class="filter-group">
              ...
              <div style="min-width:150px;">
                ...
                <select id="claim_type" name="claim_type" class="form-select">
                  ...
                  <?php foreach ($categories as $cat): ?>
                    <option value="<?= htmlspecialchars($cat) ?>" <?= $claim_type === $cat ? 'selected' : '' ?>><?= htmlspecialchars($cat) ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              <div style="min-width:150px;">
                ...
                <select id="status" name="status" class="form-select">
                  ...
                  <?php foreach ($statuses as $st): ?>
                    <option value="<?= htmlspecialchars($st) ?>" <?= $status_filter === $st ? 'selected' : '' ?>><?= htmlspecialchars(ucfirst($st)) ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              <div style="display:flex; gap:0.5rem; align-self:flex-end;">
                ...
                <a href="audit_reports.php" class="btn btn-secondary btn-sm"><ion-icon name="refresh-outline"></ion-icon> Reset</a>
              </div>
            </div>
          </form>
        </div>
      ...
      <div class="card">
        <div class="card-header">
          <ion-icon name="document-text-outline"></ion-icon> Claims Report
          <div class="ms-auto">
            <button class="btn btn-success btn-sm" onclick="exportToCSV()"><ion-icon name="download-outline"></ion-icon> Export CSV</button>
          </div>
        </div>
        <div class="card-body">
          <?php if (empty($claims)): ?>
              ...
              <table class="table" id="reportTable">
                ...
                <tbody>
                  <?php foreach ($claims as $claim): ?>
                    <tr>
                      <td><?= !empty($claim['created_at']) ? date('M d, Y', strtotime($claim['created_at'])) : 'N/A' ?></td>
                      <td>
                        <div><strong><?= htmlspecialchars($claim['employee_name'] ?? 'Unknown') ?></strong></div>
                        <small style="color: #6c757d;"><?= htmlspecialchars($claim['employee_id'] ?? '') ?></small>
                      </td>
                      <td><span class="category-badge"><?= htmlspecialchars($claim['category'] ?? 'N/A') ?></span></td>
                      <td><?= htmlspecialchars($claim['vendor'] ?? 'N/A') ?></td>
                      <td><span class="amount-badge">₱<?= number_format((float)($claim['amount'] ?? 0), 2) ?></span></td>
                      <td>
                        <?php
                          $st_cls = 'status-' . strtolower($claim['status'] ?? 'pending');
                        ?>
                        <span class="status-badge <?= $st_cls ?>"><?= htmlspecialchars(ucfirst($claim['status'] ?? 'Unknown')) ?></span>
                      </td>
                      <td><?= htmlspecialchars($claim['reviewed_by'] ?? '-') ?></td>
                      <td><?= !empty($claim['reviewed_at']) ? date('M d, Y', strtotime($claim['reviewed_at'])) : '-' ?></td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
...
function exportToCSV() {
  ...
  table.querySelectorAll('tbody tr').forEach(row => {
    ...
    row.querySelectorAll('td').forEach(td => {
      vals.push('"' + td.textContent.trim().replace(/"/g, '""') + '"');
    });
    csv += vals.join(',') + '\n';
  });
