...
$stmt = $pdo->prepare("SELECT swap_with_date, reason, status, created_at, shift_id FROM shift_swap_requests WHERE employee_id = ? ORDER BY created_at DESC");

$swap_requests = $stmt->fetchAll(PDO::FETCH_ASSOC);

// For swap request table, get shift details for each request
$shift_map = [];
if (!empty($swap_requests)) {
    $shift_ids = array_column($swap_requests, 'shift_id');
    if (count($shift_ids) > 0) {
        $in = implode(',', array_fill(0, count($shift_ids), '?'));
        $stmt = $pdo->prepare("SELECT id, shift_date, shift_type FROM shifts WHERE id IN ($in)");
        $stmt->execute($shift_ids);
...
<div class="container-fluid p-0">
  <div class="row g-0">
        ...
        <div>
          ...
          <div class="mb-4">
            ...
            <nav class="nav flex-column">
              ...
              <a class="nav-link active" href="../employee/shift_swap.php"><ion-icon name="swap-horizontal-outline"></ion-icon>Request Shift Swap</a>
            </nav>
          </div>
          <div class="mb-4">
    ...
    <div class="main-content col">
        ...
        <?php endif; ?>
        <div class="mb-4">
            <h5 class="mb-3">Your Assigned Shifts</h5>
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                          <?php
                            if (isset($shift_map[$req['shift_id']])) {
                              $s = $shift_map[$req['shift_id']];
                              echo htmlspecialchars($s['shift_date']) . ' (' . ucfirst(strtolower(date('l', strtotime($s['shift_date']))) ) . '), ' . htmlspecialchars($s['shift_type']);
                            } else {
                              echo "---";
                            }
                          ?>
                    ...
                    <tr><td colspan="5">No shift swap requests yet.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
