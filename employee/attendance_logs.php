...
/**
 * attendance_logs.php
 * Employee attendance logs with LIVE FACE DETECTION for identity verification.
 * Uses face-api.js + blink liveness verification before showing attendance records.
...
list($periodFrom, $periodTo) = getCurrentPeriod();

// ===== AJAX ENDPOINT: Fetch attendance logs =====
if (isset($_GET['action']) && $_GET['action'] === 'fetch_logs' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($emp_id)) { echo json_encode(['logs' => [], 'period_from' => $periodFrom, 'period_to' => $periodTo]); exit; }

    $stmt = mysqli_prepare($conn, "SELECT * FROM attendance WHERE employee_id = ? AND date >= ? AND date <= ? ORDER BY date DESC");
    mysqli_stmt_bind_param($stmt, 'sss', $emp_id, $periodFrom, $periodTo);
    ...
    $today = date('Y-m-d');
    $days_to_payday = (strtotime($periodTo) - strtotime($today)) / 86400;
...
<head>
  <meta charset="utf-8" />
  <title>Attendance Logs - Employee Dashboard</title>
  ...
  <style>
    * { transition: all 0.3s ease; box-sizing: border-box; }
    ...
    .face-img-thumb {
      ...
      object-fit: cover; border: 2px solid #9A66ff; background: #eee;
    }
    .status-badge {
    ...
    .period-info {
      ...
      padding: 1rem; border-radius: 8px; color: #0c4a6e;
      font-size: 0.95rem; margin-bottom: 1.2rem;
    }
    .period-info strong { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.3rem; }
    ...
    /* === LOGS SECTION TRANSITION === */
    .logs-section {
      display: none; opacity: 0; transform: translateY(30px);
    }
    .logs-section.show {
      display: block;
      animation: logsSlideIn 0.6s ease forwards;
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="topbar">
      <div>
        <h3><ion-icon name="list-outline"></ion-icon> Attendance Logs</h3>
        <small class="text-muted">Verify your identity with face detection, then view your attendance records</small>
      </div>
      <div class="profile">
    ...
    <div class="main-content">
      ...
      <div class="content-card" id="faceDetectionCard">
        ...
        <div class="info-box">
          ...
          <ul>
            ...
            <li>Once verified, your attendance logs will appear below</li>
          </ul>
        </div>
      ...
      <!-- ============================================= -->
      <!-- STEP 2: ATTENDANCE LOGS (hidden until verified)-->
      <!-- ============================================= -->
      <div class="logs-section" id="logsSection">
        <div class="content-card">
          <h5><ion-icon name="time-outline"></ion-icon> Step 2: Your Attendance Records</h5>
          ...
          <div class="period-info">
            <strong><ion-icon name="calendar-outline"></ion-icon> Current Pay Period</strong>
            <span id="periodRange"><?= htmlspecialchars($periodFrom) ?> to <?= htmlspecialchars($periodTo) ?></span>
            <span id="paydayBadge" class="ms-2 badge bg-warning text-dark" style="display:none;">Payday approaching — please submit timesheet</span>
          </div>

          <!-- Attendance Table (populated via AJAX) -->
          <div class="table-responsive" id="logsTableContainer">
            <div class="empty-state" id="logsLoading">
...
<script>
// ===================================================================
// FACE DETECTION + LIVENESS VERIFICATION FOR ATTENDANCE LOGS
// Identifies employee via face descriptor -> reveals attendance logs
// ===================================================================
...
function checkVerificationComplete() {
  if (STATE.livenessVerified && IDENTIFY.currentEmployee) {
    updateStatus('verified', 'Identity verified! (' + IDENTIFY.currentEmployee.fullname + ') — Loading your attendance logs...');
    showSuccess('Identity verified as ' + IDENTIFY.currentEmployee.fullname + '. Loading attendance records...');
    revealLogsSection(IDENTIFY.currentEmployee);
  } else if (STATE.livenessVerified) {
    updateStatus('verified', 'Liveness verified! Waiting for face identification...');
  }
}

// ===== REVEAL LOGS SECTION WITH TRANSITION =====
function revealLogsSection(emp) {
  ...
  // Fetch attendance logs via AJAX
  fetchAttendanceLogs(emp.employee_id);
}

// ===== FETCH ATTENDANCE LOGS VIA AJAX =====
function fetchAttendanceLogs(empId) {
  var container = document.getElementById('logsTableContainer');
  if (!container) return;

  var xhr = new XMLHttpRequest();
  xhr.open('POST', 'attendance_logs.php?action=fetch_logs', true);
    ...
    if (xhr.status === 200) {
      try {
        ...
        if (data.period_from && data.period_to) {
          ...
          if (periodEl) periodEl.textContent = data.period_from + ' to ' + data.period_to;
        }
        if (data.require_submit) {
          var badge = document.getElementById('paydayBadge');
          if (badge) badge.style.display = 'inline-block';
        }

        if (data.logs && data.logs.length > 0) {
          var html = '<table class="table table-bordered"><thead><tr>';
          html += '<th>Date</th><th>Time In</th><th>Time Out</th><th>Status</th>';
          html += '<th>IP In</th><th>IP Out</th><th>Face In</th><th>Face Out</th>';
          html += '</tr></thead><tbody>';

          data.logs.forEach(function(log) {
            var st = (log.status || 'Present').toLowerCase();
            var badgeClass = 'status-present';
            if (st === 'absent') badgeClass = 'status-absent';
            else if (st === 'late') badgeClass = 'status-late';

            var statusText = (log.status || 'Present');
            statusText = statusText.charAt(0).toUpperCase() + statusText.slice(1);

            // Face image paths
            var faceInPath = '../uploads/faces/' + empId + '_' + log.date + '_in.png';
            var faceOutPath = '../uploads/faces/' + empId + '_' + log.date + '_out.png';

            html += '<tr>';
            html += '<td>' + escapeHtml(log.date || '') + '</td>';
            html += '<td>' + escapeHtml(log.time_in || '-') + '</td>';
            html += '<td>' + escapeHtml(log.time_out || '-') + '</td>';
            html += '<td><span class="status-badge ' + badgeClass + '">' + escapeHtml(statusText) + '</span></td>';
            html += '<td>' + escapeHtml(log.ip_in || '-') + '</td>';
            html += '<td>' + escapeHtml(log.ip_out || '-') + '</td>';
            html += '<td><img src="' + faceInPath + '" class="face-img-thumb" alt="Face In" onerror="this.outerHTML=\'<span class=text-muted>-</span>\'"></td>';
            html += '<td><img src="' + faceOutPath + '" class="face-img-thumb" alt="Face Out" onerror="this.outerHTML=\'<span class=text-muted>-</span>\'"></td>';
            html += '</tr>';
          });
          ...
          container.innerHTML = '<div class="empty-state"><ion-icon name="calendar-outline"></ion-icon><h5>No Attendance Records</h5><p>No attendance records found for this cutoff period.</p></div>';
        }
      } catch (e) {
        container.innerHTML = '<div class="alert-box alert-danger-box">Failed to parse attendance data.</div>';
      }
    } else {
      container.innerHTML = '<div class="alert-box alert-danger-box">Failed to load attendance records.</div>';
    }
  };
  xhr.onerror = function() {
    container.innerHTML = '<div class="alert-box alert-danger-box">Network error. Could not load attendance records.</div>';
  };
...
function resetDetection() {
  ...
  if (logsSection) logsSection.classList.remove('show');
