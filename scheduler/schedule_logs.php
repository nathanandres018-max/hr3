...
// Updated join: schedule_logs.employee_id (int) references employees.id (int)
...
<head>
    <meta charset="UTF-8">
    <title>Schedule Logs - ViaHale TNVS HR3</title>
    ...
    <style>
        * { transition: all 0.3s ease; }
        body { font-family: 'QuickSand', 'Poppins', Arial, sans-serif; background: linear-gradient(135deg, #f5f7fa 0%, #fafbfc 100%); color: #22223b; font-size: 16px; }
        .sidebar { background: linear-gradient(180deg, #181818ff 0%, #1a1a2e 100%); color: #fff; min-height: 100vh; width: 220px; position: fixed; left: 0; top: 0; z-index: 1040; overflow-y: auto; padding: 1rem 0.3rem; box-shadow: 2px 0 15px rgba(0,0,0,0.1); }
        ...
        .topbar { padding: 1rem 1.5rem; display: flex; align-items: center; justify-content: space-between; gap: 1rem; background: #fff; border-bottom: 2px solid #f0f0f0; box-shadow: 0 2px 8px rgba(140,140,200,0.05); }
        .topbar .profile { display: flex; align-items: center; gap: 1.2rem; }
        .topbar .profile-img { width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 3px solid #9A66ff; }
        ...
        .dashboard-title { font-family: 'QuickSand', 'Poppins', Arial, sans-serif; font-size: 1.9rem; font-weight: 800; margin-bottom: 0; color: #22223b; letter-spacing: -0.5px; }
        .breadcrumbs { color: #9A66ff; font-size: 0.93rem; }
        ...
        .stat-card { background: linear-gradient(135deg, #fff 0%, #f8f9ff 100%); border-radius: 15px; padding: 1.5rem; box-shadow: 0 4px 15px rgba(140,140,200,0.08); border: 1px solid #f0f0f0; display: flex; align-items: center; gap: 1rem; }
        ...
        .filter-form { background: #f8f9ff; padding: 1.5rem; border-radius: 12px; margin-bottom: 1.5rem; border: 1px solid #e0e7ff; }
        .filter-form .form-select, .filter-form .form-control { border: 1px solid #e0e7ff; border-radius: 8px; padding: 0.6rem 0.8rem; font-size: 0.95rem; }
        .filter-form .form-select:focus, .filter-form .form-control:focus { border-color: #9A66ff; box-shadow: 0 0 0 0.2rem rgba(154, 102, 255, 0.15); }
        .filter-form .btn-primary { background: linear-gradient(90deg, #9A66ff 0%, #4311a5 100%); border: none; border-radius: 8px; padding: 0.6rem 1.5rem; font-weight: 600; }
        .filter-form .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(154, 102, 255, 0.4); }
        ...
        @media (max-width: 1200px) { .main-content { padding: 1.5rem 0.3rem; } .sidebar { width: 180px; padding: 1rem 0.3rem; } .main-content { margin-left: 180px; } .stats-container { grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); } }
        ...
        @media (max-width: 700px) { .dashboard-title { font-size: 1.3rem; } .main-content { padding: 0.7rem 0.2rem 0.7rem 0.2rem; } .sidebar { width: 100vw; left: -100vw; padding: 0.7rem 0.2rem; } .sidebar.show { left: 0; } .main-content { padding: 0.5rem 0.2rem; } .stats-container { grid-template-columns: 1fr; } .filter-form { padding: 1rem; } .filter-form .form-select, .filter-form .form-control { min-width: 100%; margin-bottom: 0.8rem; } .table { font-size: 0.85rem; } .table th, .table td { padding: 0.7rem 0.4rem; } }
        @media (max-width: 500px) { .sidebar { width: 100vw; left: -100vw; padding: 0.3rem 0.01rem; } .sidebar.show { left: 0; } .main-content { padding: 0.1rem 0.01rem; } .dashboard-col { padding: 1rem 0.8rem; } .dashboard-title { font-size: 1.1rem; } .topbar { flex-direction: column; gap: 0.5rem; } }
...
<div class="container-fluid p-0">
  <div class="row g-0">
        ...
        <div>
          ...
          <div class="mb-4">
            ...
            <nav class="nav flex-column">
              ...
              <a class="nav-link active" href="schedule_logs.php"><ion-icon name="reader-outline"></ion-icon>Schedule Logs</a>
    ...
    <div class="main-content col">
      <div class="topbar">
        <span class="dashboard-title">Schedule Logs</span>
        <div class="profile">
